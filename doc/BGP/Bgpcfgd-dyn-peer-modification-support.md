...
### Background:
BGP configurations via templates are rendered via 3 different files: instance.conf.j2, policies.conf.j2, peer-group.conf.j2, and if further granularity is needed for use cases, the j2 files may further branch out and invoke usecase specific j2 files/add usecase specific configuration within the same j2 file. These templates define various configurations beyond just peer config, so configurations related to to route-maps, peer-groups, prefix-lists etc.

### New changes:
An update.conf.j2 and delete.conf.j2 will be defined in the very same folder structure as above, and the exact same branch out/logic splits like the existing j2 files will apply to update.conf.j2 and delete.conf.j2. The update.conf.j2 and delete.conf.j2 will host the required logic to not only modify/delete the peers themselves, but also modify/delete associated configurations like route maps, peer groups, prefix lists etc. For the the purposes of this design we aim to simplify and only have a single j2 for update and delete each, which implies that update.j2 and delete.j2 will contain configuration that spans across all 3 types of config: instance, peer-group and policies. The alternate approach we considered but decided against was defining an update/delete version for each type of config, so we would've define templates as follows: instance.update.conf.j2, policies.update.conf.j2, peer-group.update.conf.j2, instances.delete.conf.j2, policies.delete.conf.j2, peer-group.delete.conf.j2, moving to that sort of model can be taken up in the future if the usecases for update and delete expand beyond the current one and it becomes necessary to improve the segregation of configuration for the update and delete operations.
